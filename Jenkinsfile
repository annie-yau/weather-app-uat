   ...
   stages {       
        ...
        stage('Deploy to UAT env') {
            steps {
                    ...
                    sh '''  
                        if kubectl describe service weather-app-service -n demo-env-uat; then
                            echo "Service already exists, delete first"
                            kubectl delete service weather-app-service -n demo-env-uat
                        fi
                        if kubectl describe deployment weather-app-deployment -n demo-env-uat; then
                            echo "Application already exists, delete first"
                            kubectl delete deployment weather-app-deployment -n demo-env-uat
                        fi                        
                        echo "deploy to K8S"                                  
